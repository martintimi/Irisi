...
const MALE_CATEGORIES = [
  // Native & Cultural
  { id: 'senator_kaftan', label: 'Senator Sets & Kaftans (native)', generalCat: 'native' as GarmentCategory, group: 'native' },
  { id: 'agbada_robes', label: 'Grand Agbada 3-Piece (native)', generalCat: 'native' as GarmentCategory, group: 'native' },
  { id: 'jalabiya_tunics', label: 'Jalabiya & Tunics (native)', generalCat: 'native' as GarmentCategory, group: 'native' },
  { id: 'men_caps_fila', label: 'Aso-Oke Fila & Caps (native)', generalCat: 'native' as GarmentCategory, group: 'native' },
  // Apparel
  { id: 'streetwear_hoodie', label: 'Hoodies & Sweatshirts (clothing)', generalCat: 'clothing' as GarmentCategory, group: 'clothing' },
  { id: 'suits_blazers', label: 'Suits, Tuxedos & Blazers (clothing)', generalCat: 'clothing' as GarmentCategory, group: 'clothing' },
  { id: 'tshirts_tees', label: 'T-Shirts & Graphic Tees (clothing)', generalCat: 'clothing' as GarmentCategory, group: 'clothing' },
  { id: 'shirts_polos', label: 'Luxury Shirts & Polos (clothing)', generalCat: 'clothing' as GarmentCategory, group: 'clothing' },
  { id: 'jackets_coats', label: 'Jackets & Windbreakers (clothing)', generalCat: 'clothing' as GarmentCategory, group: 'clothing' },
  { id: 'jeans_trousers', label: 'Baggy Jeans & Cargo Pants (clothing)', generalCat: 'clothing' as GarmentCategory, group: 'clothing' },
  { id: 'joggers_sweats', label: 'Joggers & Sweatpants (clothing)', generalCat: 'clothing' as GarmentCategory, group: 'clothing' },
  { id: 'shorts_sets', label: 'Shorts & Casual Sets (clothing)', generalCat: 'clothing' as GarmentCategory, group: 'clothing' },
  ...
  { id: 'men_bags_crossbody', label: 'Crossbody & Chest Rigs (bags)', generalCat: 'bags' as GarmentCategory, group: 'bags' },
  // Accessories & Jewelry
...
const FEMALE_CATEGORIES = [
  // Native & Cultural
  { id: 'boubou_kaftans', label: 'Silk Boubou, Kaftans & Abayas (native)', generalCat: 'native' as GarmentCategory, group: 'native' },
  { id: 'lace_ankara', label: 'Lace & Ankara Tailored Sets (native)', generalCat: 'native' as GarmentCategory, group: 'native' },
  // Apparel
  { id: 'dresses_gowns', label: 'Dresses, Gowns & Maxis (clothing)', generalCat: 'clothing' as GarmentCategory, group: 'clothing' },
  { id: 'two_piece_sets', label: 'Two-Piece Co-ord Sets (clothing)', generalCat: 'clothing' as GarmentCategory, group: 'clothing' },
  { id: 'corsets_tops', label: 'Tops, Corsets & Blouses (clothing)', generalCat: 'clothing' as GarmentCategory, group: 'clothing' },
  { id: 'female_streetwear', label: 'Hoodies & Sweatshirts (clothing)', generalCat: 'clothing' as GarmentCategory, group: 'clothing' },
  { id: 'women_jeans_trousers', label: 'Jeans, Cargo & Pants (clothing)', generalCat: 'clothing' as GarmentCategory, group: 'clothing' },
  { id: 'skirts_minis', label: 'Skirts & Mini Skirts (clothing)', generalCat: 'clothing' as GarmentCategory, group: 'clothing' },
  { id: 'women_shorts', label: 'Shorts & Biker Sets (clothing)', generalCat: 'clothing' as GarmentCategory, group: 'clothing' },
  { id: 'women_underwears', label: 'Underwear, Shapewear & Loungewear (clothing)', generalCat: 'clothing' as GarmentCategory, group: 'clothing' },
  // Footwear
  { id: 'women_heels_mules', label: 'Heels, Pumps & Mules (footwear)', generalCat: 'footwear' as GarmentCategory, group: 'footwear' },
  { id: 'women_slides_palms', label: 'Slides, Palms & Flats (footwear)', generalCat: 'footwear' as GarmentCategory, group: 'footwear' },
  { id: 'women_sneakers', label: 'Designer Sneakers (footwear)', generalCat: 'footwear' as GarmentCategory, group: 'footwear' },
  { id: 'women_shoes_clogs', label: 'Crocs & Foam Clogs (footwear)', generalCat: 'footwear' as GarmentCategory, group: 'footwear' },
  // Bags
  { id: 'women_bags_handbags', label: 'Handbags & Totes (bags)', generalCat: 'bags' as GarmentCategory, group: 'bags' },
  { id: 'women_bags_clutches', label: 'Clutches & Crossbody Minis (bags)', generalCat: 'bags' as GarmentCategory, group: 'bags' },
  // Accessories & Jewelry
...
const UNISEX_CATEGORIES = [
  // Apparel
  { id: 'unisex_hoodie', label: 'Streetwear Hoodies & Sweaters (clothing)', generalCat: 'clothing' as GarmentCategory, group: 'clothing' },
  { id: 'unisex_tees', label: 'Graphic Tees & Oversized Shirts (clothing)', generalCat: 'clothing' as GarmentCategory, group: 'clothing' },
  { id: 'unisex_denim', label: 'Denim Jeans & Cargo Pants (clothing)', generalCat: 'clothing' as GarmentCategory, group: 'clothing' },
  // Footwear
  { id: 'unisex_slides_palms', label: 'Slides & Palms (footwear)', generalCat: 'footwear' as GarmentCategory, group: 'footwear' },
  { id: 'unisex_shoes_clogs', label: 'Crocs & Foam Clogs (footwear)', generalCat: 'footwear' as GarmentCategory, group: 'footwear' },
  { id: 'unisex_sneakers', label: 'Sneakers & Casual Shoes (footwear)', generalCat: 'footwear' as GarmentCategory, group: 'footwear' },
  // Bags
  { id: 'unisex_bags', label: 'Crossbody Bags & Backpacks (bags)', generalCat: 'bags' as GarmentCategory, group: 'bags' },
  // Accessories & Jewelry
...
export default function MobileVendorPublish({
  ...
  const handleAddCustomColor = () => {
    ...
    if (!selectedColors.some(c => c.name.toLowerCase() === clean.toLowerCase())) {
      setSelectedColors(prev => [...prev, { name: clean, hex: customHex || resolveColorNameToHex(clean) }]);
    }
    setCustomName('');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Add photo-derived colors if not already in list
      ...
      if (enrichedColorsToSubmit.length === 0 && category !== 'accessories') {
        enrichedColorsToSubmit.push({ name: 'As Pictured', hex: '#111111', imageUrl: finalImg });
      }
        ...
        {showCustomSizeInput && category !== 'accessories' && (
          <div className="p-3 rounded-2xl bg-[var(--bg-primary)] border border-[var(--gold-accent)]/50 flex items-center gap-2 animate-fadeIn">
            <input
              ...
              className="flex-1 px-3 py-2 rounded-xl bg-[var(--bg-secondary)] border border-[var(--border-subtle)] text-xs text-[var(--text-primary)] font-bold focus:border-[var(--gold-accent)] focus:outline-none font-mono-luxury"
              autoFocus
            />
            <button
              type="button"
              onClick={handleAddCustomSize}
              className="px-3.5 py-2 rounded-xl bg-[var(--gold-accent)] text-black font-bold text-xs cursor-pointer active:scale-95 shrink-0"
            >
              Add
            </button>
            <button
              ...
              className="p-2 rounded-xl bg-[var(--bg-secondary)] text-[var(--text-muted)] hover:text-white cursor-pointer shrink-0"
            >
              <X className="h-4 w-4" />
            </button>
          </div>
        ...
        {category === 'accessories' ? (
          ...
          <div className="grid grid-cols-4 gap-2">
            {currentSizeList.map((sz) => {
              ...
              const isCustom = category === 'footwear' ? !FOOTWEAR_SIZES.includes(sz) : !APPAREL_SIZES.includes(sz);
      ...
      <div className="p-4 rounded-3xl surface-card border border-[var(--border-subtle)] space-y-3 shadow-sm font-mono-luxury text-xs">
        <div className="flex items-center justify-between">
          <span className="text-xs uppercase font-bold text-[var(--text-primary)] flex items-center gap-1.5">
            ...
            <span>4. Colorways &amp; Finishes</span>
          </span>
          <div className="flex items-center gap-2">
            <button
              type="button"
            ...
            >
              <Plus className="h-3 w-3" />
              <span>{showCustomColor ? 'Close' : '+ Custom Color'}</span>
            </button>
            <span className="text-[10px] text-[var(--text-secondary)]">
              {selectedColors.length === 0 && photoDerivedColors.length === 0 ? 'As Pictured' : `${selectedColors.length + photoDerivedColors.length} Active`}
            </span>
          </div>
        </div>
        ...
        {photoDerivedColors.length > 0 && (
          <div className="p-2.5 rounded-2xl bg-[var(--bg-secondary)] border border-[var(--border-subtle)] space-y-1">
            ...
            <div className="flex flex-wrap gap-1.5 pt-0.5">
              {photoDerivedColors.map((c) => (
                <div
                  key={c.name}
                  className="px-2 py-0.5 rounded-lg border border-emerald-500/30 bg-emerald-500/10 text-emerald-300 text-[10px] font-bold flex items-center gap-1.5"
                >
                  <span
        ...
        {showCustomColor && (
          <div className="flex items-center gap-2 p-2.5 rounded-2xl bg-[var(--bg-secondary)] border border-[var(--gold-accent)]/50 animate-fadeIn">
            <input
              type="color"
              value={customHex}
              onChange={(e) => setCustomHex(e.target.value)}
              className="h-8 w-8 rounded-lg border border-white/20 cursor-pointer bg-transparent shrink-0"
              title="Pick exact visual shade"
            />
            <input
              type="text"
              value={customName}
              ...
              placeholder="Type custom color (e.g. Sage Green, Pale Pink, Tie Dye)"
              className="flex-1 px-3 py-1.5 rounded-xl bg-[var(--bg-primary)] border border-[var(--border-subtle)] text-xs font-bold text-[var(--text-primary)] focus:outline-none focus:border-[var(--gold-accent)]"
              autoFocus
            />
            <button
              type="button"
              ...
              className="px-3.5 py-1.5 rounded-xl bg-[var(--gold-accent)] text-black font-bold text-[10px] uppercase tracking-wider shrink-0 cursor-pointer disabled:opacity-40 hover:bg-amber-400"
            >
              Add
            </button>
            <button
              ...
              className="p-1.5 rounded-xl bg-[var(--bg-primary)] text-[var(--text-muted)] hover:text-white cursor-pointer"
            >
              <X className="h-4 w-4" />
            </button>
          </div>
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex flex-wrap gap-1.5 items-center">
            ...
            <label
              title="Pick exact shade on color wheel"
